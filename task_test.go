...
func Test_NewTaskExecutor(t *testing.T) {
	...
	for k, phrase := range cases {
		t.Run(fmt.Sprint(k+1), func(t *testing.T) {
			var res string
			decorators := []pipeline.TaskDecorator{}
			if len(phrase) > 0 {
				for i := 0; i < len(phrase)-1; i++ {
					i := i
					decorators = append(decorators, func(next pipeline.TaskExecutor) pipeline.TaskExecutor {
			...
			pipeline.NewTaskExecutor(decorators)(task)
